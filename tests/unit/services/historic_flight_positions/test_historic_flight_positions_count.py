...
def test_should_serialize_get_historic_flight_positions_count_request_correctly(
    mock_get_historic_flight_positions_count_response,
):
    session = MagicMock()
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = mock_get_historic_flight_positions_count_response
    ...
    service.get_historic_positions_count(request)
    ...
    assert url == 'https://api.flightradar24.com/historic/flight-positions/count'
...
def test_should_parse_get_historical_flight_position_count_correctly(
    mock_get_historic_flight_positions_count_response,
):
    session = MagicMock()
    mock_response = MagicMock()
    ...
    response = service.get_historic_positions_count(request)
